      ...
      <div className="container">
        { this.props.graphs.map(graph => (
          <div className="comp" key={ graph.pk }>
              <Link to={`/view/${graph.pk}`}>{ graph.name }</Link> <br />
              { "Creado el " + new Date(graph.date).toLocaleDateString() } <br />
              <button onClick={this.props.deleteGraph.bind(this, graph.pk)}>Eliminar grafo</button>
          </div>
        ))}
      </div>
    )
